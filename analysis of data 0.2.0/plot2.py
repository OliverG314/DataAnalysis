...
class plot(QWidget):
    ...
    def annotate(self, hYMax=False, hYMin=False, hXMax=False, hXMin=False, showMean=False):
        ...
        meanX = sum(self.data[0])/len(self.data[0])
        meanY = sum(self.data[1])/len(self.data[1])

        self.meanPoint = {"pos": [meanX,meanY],
                          "brush": pg.mkBrush((255, 0, 0)),
                          "size": 10}

        if showMean: self.scatter.addPoints([self.meanPoint])
        else:
            try:
